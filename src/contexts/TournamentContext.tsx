...
import { Team, Match, Round, TournamentEvent } from "../types/domino";
...
const mapTeamFromDb = (dbTeam: any): Team => ({
  id: dbTeam.id,
  name: dbTeam.nome || dbTeam.name || "Time Desconhecido",
  players: [dbTeam.player1 || "Jogador 1", dbTeam.player2 || "Jogador 2"],
  capitaoId: dbTeam.capitao_id,
  createdAt: new Date().toISOString(),
  source: "manual",
  status: dbTeam.status || "CONFIRMED",
});
...
const mapMatchFromDb = (dbMatch: any, allTeams: Team[], dbMatches: any[]): Match => {
  ...
  const phase = dbMatch.fase_torneio || dbMatch.phase || getPhaseNameFromPrefix(prefix);
  const scoreA = dbMatch.placar_a || dbMatch.score_a || 0;
  const scoreB = dbMatch.placar_b || dbMatch.score_b || 0;
  const status = (dbMatch.status === 'em_andamento' ? 'LIVE' : dbMatch.status === 'encerrada' ? 'COMPLETED' : 'SCHEDULED') as Match["status"];
...
export function TournamentProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const loadFromSupabase = async () => {
      try {
        ...
        const savedEvents = await AsyncStorage.getItem("sc_events");
        const savedAuth = await AsyncStorage.getItem("sc_admin_auth");
  ...
  const saveTeams = (newTeams: Team[]) => {
    setTeams(newTeams);
    AsyncStorage.setItem("sc_teams", JSON.stringify(newTeams));
  };

  const saveMatches = async (newMatches: Match[]) => {
    setMatches(newMatches);
    AsyncStorage.setItem("sc_matches", JSON.stringify(newMatches));
    ...
    try {
      const dbMatches = newMatches.map((match) => {
        ...
        return {
          id: dbId,
          fase_torneio: match.phase,
          team_a_id: teamAId,
          team_b_id: teamBId,
          placar_a: match.scoreA,
          placar_b: match.scoreB,
          sets_a: match.setsA || 0,
          sets_b: match.setsB || 0,
          status: match.status === 'LIVE' ? 'em_andamento' : match.status === 'COMPLETED' ? 'encerrada' : 'agendada',
          torneio_id: 'default-torneio-id' // Será ajustado se pegarmos o ID real
        };
      });

      const { error } = await supabase.from("partidas").upsert(dbMatches);
  ...
  const saveEvents = (newEventList: TournamentEvent[]) => {
    ...
    AsyncStorage.setItem("sc_events", JSON.stringify(newEventList));
  };
  ...
  const addEvent = (description: string, type: TournamentEvent["type"], matchId?: string) => {
    ...
    setEvents((prev) => {
      ...
      AsyncStorage.setItem("sc_events", JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const login = (pin: string): boolean => {
    ...
    if (pin === adminPin) {
      setIsAdmin(true);
      AsyncStorage.setItem("sc_admin_auth", "true");
      return true;
    }
    return false;
  };
  ...
  const logout = () => {
    setIsAdmin(false);
    AsyncStorage.removeItem("sc_admin_auth");
  };
  ...
  const handleImportTeams = async (importedTeams: Team[]) => {
    ...
    if (newTeams.length > 0) {
      ...
      try {
        ...
        const { error } = await supabase.from("times").insert(dbTeams);
        if (error) throw error;
      } catch (err) {
  ...
  const handleDeleteTeam = async (id: string) => {
    ...
    try {
      ...
      const { error: teamError } = await supabase.from("times").delete().eq("id", id);
      ...
      if (hasMatches) {
        ...
        setMatches([]);
        AsyncStorage.removeItem("sc_matches");
        await supabase
          .from("partidas")
          .delete()
      ...
      Alert.alert("Aviso", "Não foi possível apagar esta dupla. Tente resetar o chaveamento primeiro.");
    }
  };
  ...
  const handleLoadMockTeams = async (count: number) => {
    ...
    try {
      ...
      await supabase.from("times").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    ...
    AsyncStorage.setItem("sc_teams", JSON.stringify(mockWithUuids));
    setMatches([]);
    AsyncStorage.removeItem("sc_matches");
    
    const clearedEvents: TournamentEvent[] = [];
    setEvents(clearedEvents);
    AsyncStorage.setItem("sc_events", JSON.stringify(clearedEvents));
